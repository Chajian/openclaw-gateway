...
import type { SiteConfig, Store, SitesConfig } from "@openclaw/core";
...
interface EvaluateOkResult {
  ok: true;
  value: unknown;
}

interface EvaluateErrResult {
  ok: false;
  error: string;
}

type EvaluateResult = EvaluateOkResult | EvaluateErrResult;

interface ProbeMatch {
  targetId: string;
  probe: ProbeAuthResult;
}
...
interface OnboardManualRequired {
  siteId: string;
  status: "manual_required";
  reason: string;
  accessToken?: never;
  userId?: never;
  accessTokenSecret?: never;
  userIdSecret?: never;
  currentUrl?: never;
  host?: never;
}

interface OnboardFailed {
  siteId: string;
  status: "failed";
  reason: string;
  accessToken?: never;
...
type OnboardSiteResult = OnboardSuccess | OnboardManualRequired | OnboardFailed;

interface OnboardResultRecord {
  siteId: string;
  status: string;
  reason: string;
  accessTokenSecret: string;
...
interface OnboardPayload {
  ...
  results: OnboardResultRecord[];
}

interface AttemptOnboardOpts {
  cliPath: string;
...
interface ProbeAcrossTargetsOpts {
  cliPath: string;
  browserProfile: string;
...
interface LinuxdoOnboardReport {
  items?: Array<{
    siteId: string;
...
function hostFromUrl(urlString: string): string {
  try {
...
function parseMaybeJson(stdout: unknown): unknown {
...
async function safeEvaluateOnTarget({ cliPath, browserProfile, targetId, fn }: SafeEvaluateOpts): Promise<EvaluateResult> {
  try {
...
async function openBrowserTab(cliPath: string, browserProfile: string, initialUrl: string): Promise<string> {
  ...
  const parsed = parseMaybeJson(raw) as OpenTabResult | null;
  if (!parsed?.targetId) {
...
async function listBrowserPageTabs(cliPath: string, browserProfile: string): Promise<BrowserTabEntry[]> {
  try {
    const raw = await runBrowserCommand(cliPath, browserProfile, ["--json", "tabs"]);
    const parsed = parseMaybeJson(raw) as TabsResult | null;
...
function buildLoginUrls(baseUrl: string): string[] {
...
function selectSitesForOnboarding(
  config: SitesConfig,
  report: LinuxdoOnboardReport | null,
  store: Store | null,
  includeAll: boolean
): SiteConfig[] {
  const reportMap = new Map<string, { siteId: string; onboardingNeeded?: boolean; [key: string]: unknown }>();
  ...
  for (const site of config.sites || []) {
    ...
    const accessTokenSecret = (site.auth?.accessTokenSecret as string | undefined) || `${site.id}_access_token`;
...
function buildTargetCandidates(tabs: BrowserTabEntry[], primaryTargetId: string, siteHost: string): BrowserTabEntry[] {
  const out: BrowserTabEntry[] = [];
  const seen = new Set<string>();
  const pushTab = (tab: BrowserTabEntry): void => {
...
async function probeAcrossTargets({
  cliPath,
  browserProfile,
  primaryTargetId,
  siteHost,
  approve
}: ProbeAcrossTargetsOpts): Promise<ProbeMatch | null> {
  ...
  for (const tab of candidates) {
    ...
    const probe = probeRes.value as ProbeAuthResult;
...
async function attemptOnboardSite(site: SiteConfig, opts: AttemptOnboardOpts): Promise<OnboardSiteResult> {
  const accessTokenSecret = (site.auth?.accessTokenSecret as string | undefined) || `${site.id}_access_token`;
  const userIdSecret = (site.auth?.userIdSecret as string | undefined) || `${site.id}_user_id`;
  ...
  for (const loginUrl of loginUrls) {
    ...
    const clickResult = clickRes.ok ? (clickRes.value as ClickOauthResult) : null;
...
export async function onboardLinuxdoOauthSites(argv: string[] = process.argv.slice(2)): Promise<OnboardPayload> {
  ...
  const outPath = path.resolve(cwd, (args.out as string | undefined) || "data/linuxdo-onboard-report.json");
  const cliPath = (args["openclaw-cli"] as string | undefined) || "C:\\Users\\KSG\\openclaw\\dist\\index.js";
  const browserProfile = (args["browser-profile"] as string | undefined) || "openclaw";
  ...
  const siteFilter = new Set(splitCsv((args["site-id"] as string | undefined) || (args.site as string | undefined) || ""));
  ...
  const config = await readJsonFile(configPath) as SitesConfig;
  let report: LinuxdoOnboardReport | null = null;
  try {
    report = await readJsonFile(reportPath) as LinuxdoOnboardReport;
  } catch (err) {
  ...
  const store: Store | null = masterKey ? await createStore(storePath, masterKey) : null;
  ...
  const results: OnboardResultRecord[] = [];
  for (const site of targets) {
    try {
      ...
      results.push({
        ...
        accessTokenSecret: result.accessTokenSecret || (site.auth?.accessTokenSecret as string | undefined) || `${site.id}_access_token`,
        userIdSecret: result.userIdSecret || (site.auth?.userIdSecret as string | undefined) || `${site.id}_user_id`,
        currentUrl: result.currentUrl || ""
      });
      info("onboard result", `${site.id} ${result.status}`);
    } catch (err) {
      warn("onboard error", `${site.id}: ${(err as Error).message}`);
      results.push({
        siteId: site.id,
        status: "failed",
        reason: (err as Error).message,
        accessTokenSecret: (site.auth?.accessTokenSecret as string | undefined) || `${site.id}_access_token`,
        userIdSecret: (site.auth?.userIdSecret as string | undefined) || `${site.id}_user_id`,
        currentUrl: ""
      });
    }
  }
...
// CLI self-execution
if (process.argv[1] && process.argv[1].includes("onboard-linuxdo")) {
  onboardLinuxdoOauthSites().catch((err) => {
    error("onboard failed", (err as Error).message);
    process.exitCode = 1;
  });
}
