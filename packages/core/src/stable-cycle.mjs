...
async function syncSites(config, store, createAdapterFn) {
  const results = [];
...
async function runOnce(opts, createAdapterFn) {
...
async function main() {
  const { createPluginHost } = await import("./plugin-host.mjs");
  ...
  const openclawConfigPath = path.resolve(args["openclaw-config"] || path.join(cwd, "config", "openclaw.json"));
  ...
  const createAdapterFn = (site, store) => host.adapters.create(site, store);
  const attempts = [];
