...
import type { AdapterSyncResult } from "../types.js";
...
export class MockPublicSiteAdapter extends AdapterBase {
  async sync(): Promise<AdapterSyncResult> {
    ...
    const seed = (settings.seed as string) || this.site.id;
    const tokenRef = (this.site.auth as Record<string, unknown> | undefined)?.tokenSecret as string || "not-configured";
