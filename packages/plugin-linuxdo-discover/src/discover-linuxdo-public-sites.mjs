...
function extractTopicId(topicUrl) {
  try {
...
function buildTopicRssUrl(topicUrl) {
...
function guessBaseUrl(urlString) {
  try {
    const u = new URL(urlString);
...
function toTopicLink(urlString) {
  try {
    const u = new URL(urlString);
    const matched = u.pathname.match(/\/t\/(?:[^/]+\/)?(\d+)/i);
    if (!matched?.[1]) {
...
async function discoverTopicsViaBrowser({ cliPath, browserProfile, keywords, maxTopics }) {
  const pages = [
  ...
  for (const page of pages) {
    ...
    for (const item of Array.isArray(list) ? list : []) {
      const link = toTopicLink(item.link || "");
      if (!link) {
        continue;
      }
      const keywordHits = matchKeywords(item.title || "", keywords);
...
function buildSiteListFromTopics(topics, excludedHosts) {
...
export async function discoverLinuxdoPublicSites(argv = process.argv.slice(2)) {
  const args = parseArgs(argv);
  const cwd = process.cwd();
  const outPath = path.resolve(cwd, args.out || "data/linuxdo-public-sites.json");
  const feeds = splitCsv(args.feeds).length ? splitCsv(args.feeds) : DEFAULT_FEEDS;
  const keywords = splitCsv(args.keywords).length ? splitCsv(args.keywords) : DEFAULT_KEYWORDS;
  ...
  const openclawCli = args["openclaw-cli"] || "C:\\Users\\KSG\\openclaw\\dist\\index.js";

  const excludedHosts = new Set(DEFAULT_EXCLUDED_HOSTS);
  for (const host of splitCsv(args["exclude-hosts"])) {
  ...
  for (const feedUrl of feeds) {
    try {
      ...
      warn("feed failed", `${feedUrl}: ${err.message}`);
    }
  }
  ...
  if (feedOkCount > 0 && topicMap.size > 0) {
    ...
    for (const topic of candidateTopics) {
      ...
      try {
        ...
        warn("topic rss failed", `${topicRssUrl}: ${err.message}`);
      }
    }
...
if (process.argv[1] && process.argv[1].includes("discover-linuxdo")) {
  discoverLinuxdoPublicSites().catch((err) => {
    error("discover failed", err.message);
    process.exitCode = 1;
  });
}
